...
import { Table, Popconfirm, Switch, notification } from 'antd';
...
export default function TaskList() {
  ...
  return <Observer>{() => {
    return <Table className="box" dataSource={local.tasks} rowKey="resource_id" scroll={{ y: 600 }} loading={local.isLoading} pagination={{
      pageSize: 20,
      ...
      <Column title="操作" width={100} dataIndex="action" key="action" align="center" render={(text, record) => (
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-evenly' }}>
          ...
          <Popconfirm title="确定?" icon={<WarningOutlined />} onConfirm={() => { destroyTask(record.toJSON()) }}>
            <DeleteOutlined />
          </Popconfirm>
        </div>
      )} />
