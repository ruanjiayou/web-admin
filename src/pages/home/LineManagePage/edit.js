...
export default function ({ data, save, cancel }) {
  ...
  return <Observer>{() => <Modal
    ...
    width={600}
    title={local.data._id ? '修改链接' : '添加链接'}
    onCancel={e => { cancel() }}
    onOk={async () => {
  ...
  >
    <Form>
      ...
      <Form.Item style={{ marginBottom: 5 }} label="环境" labelCol={lb} wrapperCol={rb}>
        <Input placeholder="环境" value={local.data.env} onChange={e => local.data.env = e.target.value} />
      </Form.Item>
