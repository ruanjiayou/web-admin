...
export default function ChannelList({ isLoading, items, ...restProps }) {
  ...
  return <div style={{ backgroundColor: '#eee', flex: 1, margin: '0 15%', height: '100%', overflow: 'auto' }}>
    <Observer>{() => {
      if (isLoading) {
        ...
        // return items.map(data => <ChannelItem data={data} key={data.id} {...restProps} />)
                ...
                {items.map((item, index) => (
                  <Draggable key={item.id} draggableId={item.id} index={index}>
                    {(provided, snapshot) => (
                      ...
                      >
                        <ChannelItem data={item} key={item.id} {...restProps} />
                      </div>
                    )}
                  </Draggable>
                ))}
